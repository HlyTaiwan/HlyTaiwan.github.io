...
Original file is located at
    https://colab.research.google.com/drive/1f2OpTgPtP3YAXo5hQZBHtH-HorgzGd2-
"""
...
# 把"TfidfVectorizer"轉成另一變數"tfidf_vectorizer"，該變數不會計算stopword
# 雖然"TfidfVectorizer"有"stop_words"的default，但因為不清楚裡面包含哪些stopword，我選擇用nltk套件的stopword
tfidf_vectorizer = TfidfVectorizer(stop_words=stopwords.words('english'))
# 將每篇新聞(即"all_news"的每個string)的單字計算tf-idf，然後輸出向量
tfidf_vectors = tfidf_vectorizer.fit_transform(all_news)
...
# tfidf_vector包含所有1095文本的向量，區分training data和test data前先轉成list
tfidf_vectors_array = tfidf_vectors.toarray()
# X為training data向量的numpy array
X = np.array([tfidf_vectors_array[id-1] for id in training_new_docID])
# y為training data類別的numpy array
y = np.array(training_new_classID)
# test為training data的numpy array
test = np.array([tfidf_vectors_array[id-1] for id in test_new_docID])

# 設立一個Support Vector Classification，kernel為linear
...
# 使用 one vs rest 的方式對每個類別畫圖
for class_id in range(13):
  # 以第一圈為例，若是class 1，y當中是1的元素為1，其餘為0。
  # e是真正類別的二元化list
  e = [1 if id == class_id + 1 else 0 for id in expected_y_results]
  # p是模型預測類別的二元化list
  p = [1 if id == class_id + 1 else 0 for id in predicted_y_results]
  # 計算precision和recall
  precision, recall, thresholds = metrics.precision_recall_curve(e, p)
  # 畫出每一條precision-recall curve
  plt.plot(recall, precision, color = colors[class_id], label=f'class{class_id+1}')
...
# 輸出成符合格式的csv檔
pa2_svmLinear = pd.DataFrame({'Id':test_new_docID, 'Value':predicted_test_results})
pa2_svmLinear.to_csv('pa2_svmLinear.csv', index = False)